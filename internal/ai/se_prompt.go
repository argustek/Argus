...
文档处理工具（处理PDF/Word/Excel等非代码文件）：
- read_pdf: 读取PDF文件。普通文本型PDF直接提取文字；扫描版/图片型PDF设置use_ocr=true启用OCR识别（需要Tesseract，会自动检测并提示安装）
- read_docx: 读取Word文档(.docx)，提取段落文本和表格数据
- write_docx: 生成Word文档(.docx)，支持标题(h1/h2/h3)、正文、列表、表格、分页符等格式
- compare_docs: 比较两个文档（PDF vs PDF / DOCX vs DOCX / PDF vs DOCX），输出unified diff差异报告

工具自举系统（自动安装缺失依赖）：
- ensure_tool: 检测某功能的依赖是否就绪，缺失时自动安装。例如：ensure_tool("read_pdf_ocr") 会检查并安装OCR相关依赖
- install_pkg: 通过包管理器安装任意软件包。支持 pip/npm/cargo/go
...
=== 输出规则（严格执行） ===
- 每次回复不超过3句话。直接给结果，不要解释过程，不要客套
- Report results in natural language, like a developer syncing with a teammate. No tool names or argument dumps.
- Bad: ✅ write_file main.go (71 bytes) / ✅ exec 'go run main.go' / Hello World
- Good: Created main.go and ran it — output: Hello World
- 不要问"要不要继续"，不要给选项，一句话能说清的绝不说两句
`
...
func (s *SEProcessor) toolCallToSEAction(tc ToolCall) SEAction {
	...
	case "read_pdf":
		...
		if v, ok := args["use_ocr"].(bool); ok {
			action.UseOCR = v
		}
		action.Type = "read_pdf"
	case "read_docx":
		if v, ok := args["path"].(string); ok {
			action.Path = v
		}
		action.Type = "read_docx"
	case "write_docx":
		if v, ok := args["path"].(string); ok {
			action.Path = v
		}
		...
		action.Type = "write_docx"
	case "compare_docs":
		if v, ok := args["path_a"].(string); ok {
			action.Path = v
		}
		if v, ok := args["path_b"].(string); ok {
			action.ComparePathB = v
		}
		action.Type = "compare_docs"
	// ========== 工具自举 ==========
	case "ensure_tool":
		if v, ok := args["tool_name"].(string); ok {
			action.ToolName = v
		}
		action.Type = "ensure_tool"
	case "install_pkg":
		if v, ok := args["pkg_name"].(string); ok {
			action.PkgName = v
		}
		if v, ok := args["pkg_manager"].(string); ok {
			action.PkgManager = v
		} else {
			action.PkgManager = "pip" // 默认 pip
		}
		action.Type = "install_pkg"
	}
...
var SETools = []Tool{
	...
	// ========== 文档处理工具 ==========
	{
		...
		Function: ToolFunction{
			Name:        "read_pdf",
			Description: "读取PDF文件内容。支持普通文本型PDF（基于PyMuPDF）和扫描图片型PDF（OCR，需Tesseract）。自动提取每页文本，返回完整内容。对于扫描版/图片型PDF，设置use_ocr=true启用OCR识别。",
			Parameters: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"path": map[string]interface{}{
						"type":        "string",
						"description": "PDF文件路径（相对于工作目录，如 docs/report.pdf）",
					},
					"use_ocr": map[string]interface{}{
						"type":        "boolean",
						"description": "是否使用OCR模式。默认false。当PDF是扫描件、截图或无法提取文字时设为true（需要Tesseract-OCR，会自动检测并提示安装）",
					},
				},
				"required": []string{"path"},
			},
	...
	},
	{
		...
		Function: ToolFunction{
			Name:        "read_docx",
			Description: "读取Word文档(.docx)内容。提取段落文本（保留样式信息）、表格数据。返回完整的文档结构化内容。",
			Parameters: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"path": map[string]interface{}{
						"type":        "string",
						"description": "Word文档路径（相对于工作目录，如 docs/spec.docx）",
					},
				},
				"required": []string{"path"},
			},
	...
	},
	{
		...
		Function: ToolFunction{
			Name:        "write_docx",
			Description: "生成/写入Word文档(.docx)。支持标题(h1/h2/h3)、正文、有序列表、无序列表、表格、分页符等格式。content参数为JSON数组，描述文档结构。",
			Parameters: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"path": map[string]interface{}{
						"type":        "string",
						"description": "输出文件路径（相对于工作目录，如 output/report.docx）",
					},
					"content": map[string]interface{}{
						"type":        "string",
						"description": "文档内容的JSON数组。示例：[{\"type\":\"heading\",\"text\":\"标题\",\"level\":1},{\"type\":\"paragraph\",\"text\":\"正文内容\"},{\"type\":\"table\",\"rows\":[[\"列1\",\"列2\"],[\"值1\",\"值2\"]]}]。支持的type: heading/paragraph/bullet_list/numbered_list/table/page_break",
					},
				},
				"required": []string{"path", "content"},
			},
	...
	},
	{
		...
		Function: ToolFunction{
			Name:        "compare_docs",
			Description: "比较两个文档并输出差异报告。支持 PDF vs PDF、DOCX vs DOCX、PDF vs DOCX 等任意组合。使用 unified diff 格式输出差异，包含页码/行号定位。",
			Parameters: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"path_a": map[string]interface{}{
						"type":        "string",
						"description": "第一个文件路径（基准文档）",
					},
					"path_b": map[string]interface{}{
						"type":        "string",
						"description": "第二个文件路径（对比文档）",
					},
				},
	...
	// ========== 工具自举系统 ==========
	{
		...
		Function: ToolFunction{
			Name:        "ensure_tool",
			Description: "检测工具依赖是否就绪，缺失时自动安装。用于确保执行某操作所需的Python库或系统工具已安装。支持的工具名：read_pdf, read_pdf_ocr, read_docx, write_docx, compare_docs。调用后会自动检查依赖并尝试安装。",
			Parameters: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"tool_name": map[string]interface{}{
						"type":        "string",
						"description": "要确保可用的工具名称（如 read_pdf_ocr 表示带OCR的PDF读取能力）",
					},
				},
	...
	},
	{
		...
		Function: ToolFunction{
			Name:        "install_pkg",
			Description: "通过包管理器安装软件包。支持 pip(Python)、npm(Node.js)、cargo(Rust)、go(Go)。用于安装AI判断需要的任何依赖。",
			Parameters: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"pkg_name": map[string]interface{}{
						"type":        "string",
						"description": "要安装的包名（如 pandas, openpyxl, requests）",
					},
					"pkg_manager": map[string]interface{}{
						"type":        "string",
						"description": "包管理器: pip(默认), npm, cargo, go",
						"enum":        []string{"pip", "npm", "cargo", "go"},
					},
				},
