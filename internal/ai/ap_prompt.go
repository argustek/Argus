...
=== 输出规则（严格执行） ===
- 每次回复不超过3句话。直接给结论（通过/驳回），不要解释理由过程
- 结论格式保持上方 JSON 格式即可，不要额外文字
...
🆕 [FIX-20260607] **关键自检规则（防止失职！）**：
- ⚠️ **每个任务必须自检：任务目标达成了吗？**
- ⚠️ 对于 "清理/删除/整理" 类任务：必须用 list_files 查看清理后文件列表，确认垃圾文件已被删除。如果还有大量 test_*/hello_*/debug_* 等垃圾 → ❌ REJECT
- ⚠️ 对于 "创建/修改" 类任务：必须用 read_file 查看文件内容，确认不是空文件。用 exec 验证编译/运行是否通过
- ⚠️ **如果PM说"✅ APPROVED"，不要被带偏 — 你是独立审查者，必须自己验证！**

📋 **文档树审计（如项目使用了 .argus 文档树）**：
- 用 verify_doc_exports 检查文档的 exports 与代码是否一致
- 检查被修改模块的文档是否已同步更新
- 确认 CHANGELOG.md 是否有本次变更记录
`
...
var APTools = []Tool{
	...
	},
	{
		Type: "function",
		Function: ToolFunction{
			Name:        "verify_doc_exports",
			Description: "验证文档的 exports 字段与代码中的实际导出符号是否一致。读取 code_ref 指向的代码文件，解析导出项并与文档对比，返回差异列表。",
			Parameters: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
	...
	},
	{
		...
		Function: ToolFunction{
			Name:        "check_impact",
			Description: "查询修改指定文档会影响哪些其他文档。在审核时判断是否有关联模块需要同步更新。",
			Parameters: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"doc_id": map[string]interface{}{
...
func (p *APProcessor) executeTool(name string, argsJSON string) string {
	...
	case "verify_doc_exports":
		...
		result, err := doclib.VerifyDocExports(p.workDir, args.DocID)
		if err != nil {
			return fmt.Sprintf("验证失败: %v", err)
		}
		if result.Match {
			return fmt.Sprintf("✅ %s", result.Detail)
		}
		return fmt.Sprintf("⚠️ 不一致发现:\n%s", result.Detail)
	...
	case "check_impact":
		...
		tree, err := doclib.LoadCache(p.workDir)
		if err != nil {
			tree, err = doclib.BuildTree(p.workDir)
			if err != nil {
