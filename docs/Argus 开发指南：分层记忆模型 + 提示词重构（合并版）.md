...
| 文件 | 操作 | 状态 |
|------|------|------|
| `internal/ai/pm_prompt.go` | 替换 `PMPrompt` 常量为英文核心版（~50行） | ✅ 已完成 |
...
| `internal/core/argus.go` | 移除 `seExecutionSatisfied` 中的 `exit status` / `command failed` 硬检查 | ✅ 已完成 |
| `internal/core/argus.go` | 修改 `pmDirectExecute` 重试提示为通用版（不再硬编码 coding 场景） | ✅ 已完成 |
| `internal/core/argus.go` | 删除硬编码关键词启发式检测（hello world / fibonacci 等），改由 PM 决策树判断 | ✅ 已完成 |
...
- [x] 移除 `seExecutionSatisfied` 中的 `exit status` / `command failed` 硬检查
- [x] 修改 `pmDirectExecute` 重试提示为通用版（不硬编码 coding 场景）
- [x] 删除 `argus.go` 中的硬编码关键词启发式检测（由 PM 决策树替代）
